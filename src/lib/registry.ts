...
// Cabinets whose whole identity is one external service. Categorized by slug,
// not by agent department: a Gmail cabinet's agents are "general" clerks, so
// department-derivation would scatter these across the business sections and
// bury the one browsing question integrations answer — "does it work with
// the thing I use?"
...
const DEPARTMENT_SECTIONS: Record<string, string> = {
  ...
  evaluation: "Product & Engineering",
  ...
  general: "Operations & Finance",
  root: "Operations & Finance",
  executive: "Leadership",
  leadership: "Leadership",
};

const FALLBACK_SECTION = "Life & Learning";

// A cabinet's section = the browse section of its most common agent department.
export function sectionForAgents(agents: AgentMeta[]): string {
  ...
  return DEPARTMENT_SECTIONS[top] ?? FALLBACK_SECTION;
}
